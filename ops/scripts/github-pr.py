...
def _load_tier0() -> list[dict]:
    ...
    try:
        ...
        return []
    return [r for r in (data.get("active") or []) if isinstance(r, dict) and r.get("repo")]
...
def _converged_target_repos() -> tuple[list[str], dict[str, int]]:
    """Tier 0 + Tier 1, deduped, BIG_ORGS-filtered (Tier 1 only). Returns
    (ordered_repo_list, tier_map) where tier_map[repo] is 0 or 1."""
    tier0 = _load_tier0()
    tier1 = _load_tier1()
    repos: list[str] = []
    tier: dict[str, int] = {}
    ...
    for r in tier1:
        ...
        repos.append(slug)
        tier[slug] = 1
    return repos[:18], tier  # cap per Repos.md §Discovery flow
...
def get_claimable_issues():
    ...
    """
    targets, tier_map = _converged_target_repos()
    all_issues = []
...
def tier_score(issue: dict) -> int:
    """Per Repos.md §Discovery flow: weight Tier 0 highest, Tier 1 secondary,
    plus issue-level signals. Used to sort the candidates so the cron picks
